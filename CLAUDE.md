...
node scripts/build-character-layers.mjs     # corta mc-idle.png en capas recoloreables (paper doll del MC)
```
...
- **El MC es un paper doll cortado de su propio sprite** (Fase 10, 2ª pasada). `scripts/build-character-layers.mjs` parte `mc-idle.png` en 11 capas de material hacia `public/assets/characters/layers/`; `src/ui/characterDraw.ts` las compone en una **textura canvas cacheada** por look, recoloreando cada capa al remapear su luminancia sobre una rampa de 4 valores (`src/data/character.ts`) entre los stops medidos (`src/data/characterLayers.ts`). Así la piel, la tenida y el pelo cambian **sobre el dibujo original** en vez de imitarlo. Pelo y ojos abiertos no existen en el sprite (usa gorra y lentes) y se trasplantan de `rival-idle.png`, que es el mismo dibujante. Tres intentos previos de dibujarlo proceduralmente (rects, string-art, perfiles generados) se descartaron por verse baratos; **no volver a ese camino**. El arte pendiente por slot (cortes, barba, aros, cadenas, hoodie, pantalón) está listado en `docs/ASSETS.md`.
...
- **Tu vida te define (Fase 10):** cada acción mueve los ejes de identidad (`src/data/identityDrift.ts` + `driftFromAction`), con rendimientos decrecientes **solo hacia afuera** (volver al centro no se frena, así el eje lee la proporción de tu vida) y **techo de 70** para que los dilemas sigan pesando. Una carrera con batallas y disco cerca del centro se lee como **MC completo** (atractor con `deeds`, gana sobre los que miran solo ejes). Grabar construye una obra (`src/data/releases.ts`: sencillo → EP → disco → gira → sello) y la rama musical se bifurca en **Productor** (underground) y **Artista de discos** (hacia afuera). Un jugador puede llegar a artista famoso del rap sin ser batallero.
...
- **La carrera abre con un prólogo (Fase 12 E):** al tocar COMENZAR, el modo es `"intro"` (`src/scenes/IntroScene.ts`): una batalla real contra un rival fijo de la pieza (`IntroConfig.rivalName`), la noche antes del día uno, así que no gasta energía, no mueve el reloj y casi no paga (`finishIntroBattle`). Deja la rivalidad registrada, y ese es el gancho. Enter sube a la tarima y Esc lo salta; saltarlo no consume RNG, así que la carrera es idéntica. Los textos están en `src/data/intro.ts`. **Los escenarios de trazas y `playthrough.mjs` presionan Escape después del primer Enter**; el escenario `prologue` juega el prólogo entero.
- **La pieza es siempre la base y crece con la carrera (Fase 12 C):** la pantalla base ya no cambia de fondo por etapa (la etapa se ve en las batallas). `src/data/roomProps.ts` lista qué gana la pieza y con qué: compras (`item`), discos (`release`), victorias (`wins`), fans y rango de un stat; `RoomSystem.earnedRoomProps` lo deriva del estado (no se guarda, así un save viejo muestra lo que ya ganó) y `src/scenes/roomLife.ts` lo dibuja. Aparecer un prop nuevo dispara el cartel "TU PIEZA CAMBIO".
- **Los stats se muestran como rangos (Fase 12 C):** Novato → Aprendiz → Callejero → Filoso → Veterano → Maestro → Leyenda (`ProgressionConfig.skillTiers`, `skillTier()`), con una marca por punto dentro del rango. Nunca "n / 99": el rival más fuerte del juego tiene flow 10.
