...
- **El MC es modular (Fase 10):** una **pila de capas** de data de píxeles (`src/data/character.ts`, grilla 24×56) compuesta por `src/ui/characterDraw.ts` — piel, corte, barba, tenida y **los accesorios comprados** (las piezas se indexan por el id del ítem de la tienda). Todas las pantallas lo dibujan por `mcFigure`/`addMcFigure`, así que el personaje que armas es el que aparece en todas. Está hecho para reemplazarse por sprites sin tocar el cableado. La **barbería** (`BarberSystem`, tecla **P**) vende corte, barba y color.
